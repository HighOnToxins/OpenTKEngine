...
public struct Shape
{
    ...
    public Shape(Vector3 position, Vector3 size, Vector3 normal, float rotation, Vector4 color) :
        this(position, size, Quaternion.FromAxisAngle(normal, rotation), color)
    { }

    public Shape(Vector3 position, Vector3 size, Vector4 color) :
        this(position, size, Quaternion.Identity, color)
    { }

    public Shape(Vector2 position, Vector2 size, float rotation, Vector4 color) : 
        this(new Vector3(position, 0), new Vector3(size, 1), Vector3.UnitZ, rotation, color)
    { }

    public Shape(Vector2 position, Vector2 size, Vector4 color) :
